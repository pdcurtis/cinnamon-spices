...
<div class="cs-comment-box">
    ...
    <?php if ($this->session->userdata('oauth')) { ?>
        ...
        <div class="cs-comment-form">
            ...
            <input name="parent_id" value="1" id="parent_id" style="display: none;"/>
        ...
        <?php foreach ($comments->result() as $comment):
            ...
            <div class="cs-media cs-comment-row">
                <div class="cs-media-image cs-comment-image">
                    <?= anchor($comment->user_link, "<img alt=' $comment->user_full_name ' src='$comment->user_avatar' class='avatar avatar-50 photo'/>", "target='_blank' class='url'") ?>
                </div>
                <div class="cs-media-content cs-flex-column cs-flex-grow">
                    <div class="cs-comment-author cs-flex-row">
                        <span class="cs-comment-author-name">
                            <?= anchor($comment->user_link, $comment->user_full_name, "target='_blank' class='url'") ?>
                        </span>
                        -
                        <span class="cs-comment-date" style="margin-right: 1rem;" title='<?= $time_actual ?>'>
                            <?= $time_span ?>
                        </span>
                        -
                        <span style="margin-left: 1rem;">
                            <?= anchor("#", 'Reply', "data-id='$comment->id' class='reply'") ?>
                        </span>
                    </div>
                    <div class="cs-comment-text">
                        <?= str_replace("\n","<br>\n",htmlspecialchars(trim($comment->message))) ?>
                    </div>
                </div>
            </div>
