<div class="cs-header-menu">
    ...
    <?php if (!$this->dx_auth->is_logged_in()) { ?>
        <!-- <a class="cs-header-menu-link cs-header-menu-login <?= preg_match('|^/auth/login.*?|',$_SERVER['REQUEST_URI'])?'active':'' ?>" href="/auth/login">Log In</a>
        <a class="cs-header-menu-signup cs-button cs-button-thin-text" href="/auth/register">Sign Up</a> -->
    <?php } else { ?>
        <!-- <a class="cs-header-menu-link cs-header-menu-login <?= preg_match('|^/users/change_details.*?|',$_SERVER['REQUEST_URI'])?'active':'' ?>" href="/users/change_details">My Profile</a>
